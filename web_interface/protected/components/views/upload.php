...
<div id="<?php echo $id;?>" style="position:relative">
	...
	<form method="post" style="display:none" action="<?php echo $uploadTo;?>" enctype="multipart/form-data">
		...
		*/?>
		...
		<input type="file" name="<?php echo $filename?>" style="width:75px;position:absolute;top:0;left:0;filter:alpha(opacity=0);opacity:0;z-index:999" class="<?php echo $filename?>" id="<?php echo $filename?>"></input>
	</form>
...
<script type="text/javascript">
	...
	//选择了文件就上传 
		...
		//return;
		//检查大小
		if((document.getElementById("<?php echo $filename?>") != null) && (document.getElementById("<?php echo $filename?>").files != null))
		{//fucking ie
			//alert(document.getElementById("<?php echo $filename?>").files[0].size);
			//return;
			if(document.getElementById("<?php echo $filename?>").files[0].maxSize > <?php echo $maxSize;?>)
			{
				<?php echo $id?>error("filesize too large");
				return false;
			}
			if(document.getElementById("<?php echo $filename?>").files[0].size > <?php echo $maxSize;?>)
			{
				<?php echo $id?>error("filesize too large");
				return false;
			}
		...
		//alert(types.length);
		var append = <?php echo $id?>getAppendix(checkVal);
		if(<?php echo $id?>notIn(append,types))
		{
			<?php echo $id?>error("file type wrong");
			return false;
		}
		//alert("c");
		<?php echo $id?>error("");
