...
cw.ec("#cIndex > div.datasetList > a.block > div.addonBlock",function(e){
	...
	$(document).ready(function(){
		...
		data.forLabeling = <?php echo $forLabeling?>;
		goTo("datasetVideos");
		$("#siteHeader > input.loading").change();
		$("#cIndex > div.datasetVideos").html("");
		$("#cIndex > input.datasetId").val(data.datasetId);
			cw.post("<?php echo Yii::app()->baseUrl;?>/index.php/main/getDatasetVideos?start=1&limit=10&sign=0&forLabeling="+data.forLabeling,data,function(result){
	...
	function addToDatasetVideos(result)
	{
		...
		//for(var i=0;i<13;++i)
		{
			$("#cIndex > div.datasetVideos").append('<div class="block">'+
				...
				'<input class="videoname" type="hidden" value="'+result.videos[i].videoname+'"></input>'+
...
cw.ec("#cIndex > div.datasetVideos > div.block",function(){
	...
	var data = {};
	data.videoname = $(this).children("input.videoname").val();
	data.dvId = $(this).children("input.dvId").val();
	data.datasetId = $(this).children("input.datasetId").val();
	// now we go the other page
	window.open("<?php echo Yii::app()->baseUrl?>/index.php/application/cAudioSyncPairView?dvId="+data.dvId+"&forLabeling="+forLabeling+"&videoname="+encodeURIComponent(data.videoname)+"&datasetId="+data.datasetId,"_self");
	return;
...
<div class="mainTitle" style="color:gray;text-align:center;font-weight:bold;padding-top:10px"> You are at - Audio Synchronization Run - <?php echo $datasetName?>
&nbsp;&nbsp;
<a class="" href="<?php echo Yii::app()->baseUrl?>/index.php/application/cAudioSyncRunList" style="text-decoration:none">Back to Run List</a>
</div>
<div id="cIndex">
