...
var Rashomon = {
		...
		this.timeline.on("play", function () {
			...
			$(Rashomon.videos).each(function () {
				...
				if (Rashomon.timeline.currentTime() > offset && Rashomon.timeline.currentTime() < offset + duration && !$("#vcontain" + this.id).is(":hidden")) {
					this.pp.play();
					//console.log("playing video "+ this.name);
				}
			}); // end videos each
		}); //end play
	...
	setupEvents: function () {
		...
		$(".vidtl").click(function (e) {
		 ...
		 var numCurPlayingVideos = $(".container").length;
...
video.prototype = {
	setupVid: function () {
		...
		if(of == 0.0)
		{
			of = 0.000001;// # otherwise first video won't play
		}
...
/*
	...
	displayVideo: function () {
		...
		this.pp.on('timeupdate', function () {
			...
			var delay = (Rashomon.timeline.currentTime() - (offset + this.currentTime())).toFixed(2) * 1000;
			if (!Rashomon.timeline.media.paused && Math.abs(delay) > 1250) {
				this.currentTime(Rashomon.timeline.currentTime() - offset);
				Rashomon.delayFixed++;
				//console.log(Rashomon.delayFixed);
			}
...
$(document).ready(function () {
	...
	// alert('d');
		...
		//console.log(Rashomon.manifest);// .videos => {"duration","offset","videoname"}
		//console.log(Rashomon.manifest.earliest); // undefined
		Rashomon.earliest = moment(Rashomon.manifest.earliest).toDate(); // so this is the browser time now
		//console.log(Rashomon.fulldur);//0.0
		Rashomon.setupVideos();// videos are saved in Rashomon.videos
		Rashomon.setupPhotos();

		Rashomon.setupTimeline(Rashomon.fulldur);
		//console.log(Rashomon.fulldur);// a little longer than the max video lengtth
		$(Rashomon.videos).each(function () {
			this.displayVideo();
